...
public class BayesNet implements Serializable {
    ...
    public BayesNet(BayesNet bn) {
        ...
        for (int i = 0; i < bn.probability_variables.length; i++)  probability_variables[i] = bn.probability_variables[i];
        for (int i = 0; i < bn.probability_functions.length; i++)  probability_functions[i] = bn.probability_functions[i];
    ...
    public BayesNet(String s) throws IFException {
        this();
        StringReader istream = new StringReader(s);
    ...
    /**
     * Find the ProbabilityFunction that corresponds to a given ProbabilityVariable. Note: the index of a variable is
     * used by the function, as it is the only reference to the variable that is guaranteed to identify the variable
     * uniquely.
     */
