 ...
 */
public class PS2HTML implements DocFilter {
	...
	String pythonData = "import os, sys, regex, posixpath, posix \n" + "from string import split, atoi, find \n"
			...
			+ "   os.system(\"gs -q -dNODISPLAY -soutfile=%s ps2html.ps %s quit.ps\" % \n"
			...
			+ "      print \"Usage: ps2html format input [output]\" \n"
			...
			+ "if __name__ == '__main__': \n" + "   main()";
	{
		...
		try {
			File f1 = File.createTempFile("ps2html", "py");
			File f2 = File.createTempFile("ps2html", "ps");
			int index1 = pythonData.indexOf("ps2html.ps");
			pythonData = pythonData.substring(0, index1) + f2.getAbsolutePath() + pythonData.substring(index1 + 10);
			try {
				f1.delete();
			} catch (Exception e) {
				errord = true;
	...
	public PS2HTML() throws Exception {
		try {
	 ...
	 *                Description of the Exception
	 */
	public String convertPSToHTML(String s4) throws Exception {
		boolean errord = false;
		String s2 = s4;
		...
		try {
			...
			java.io.File file1 = java.io.File.createTempFile(filename, ".ps");
			java.io.File file2 = java.io.File.createTempFile(filename, ".html");
			PrintWriter out = new PrintWriter(new FileWriter(file1));
			out.print(s2);
			out.close();
			// NativeExec.execute("ps2ps -dLanguageLevel=1 " + file1.getAbsolutePath()+" _"
			// + filename + ".ps");
			NativeExec.execute("python " + scriptfile.getAbsolutePath() + " html " + file1.getAbsolutePath() + " "
			...
			if (aux.equals("<html>\n")) {
				try {
					(new java.io.File(filename + ".dat")).delete();
				} catch (Exception e2) {
					errord = true;
				}
	 ...
	 *                Description of the Exception
	 */
	public String convertPSToHTML(Reader input) throws Exception {
		...
		return convertPSToHTML(sb.toString());
	}
	 ...
	 *                Description of the Exception
	 */
	public String convertPSToHTML(InputStream input) throws Exception {
	 ...
	 *                Description of the Exception
	 */
	public String convertPSToHTML(java.io.File input) throws Exception {
	 ...
	 *                Description of the Exception
	 */
	public String convertPSToHTML(URL input) throws Exception {
